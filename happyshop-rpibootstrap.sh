...
sudo /bin/sh -e - << EOF
  ...
  debconf-set-selections <<< "mysql-server mysql-server/root_password password ${MYSQL_SERVER_PASSWORD}"
  debconf-set-selections <<< "mysql-server mysql-server/root_password_again password ${MYSQL_SERVER_PASSWORD}"
